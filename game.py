...
class Camera:
    """Desloca a visão horizontalmente seguindo o jogador."""
...
class Game:
    """Gerencia o loop principal, estados e transições de fase."""
    ...
    def __init__(self) -> None:
        ...
        # Atributos carregados em _load_level
        self.player:    Player
        self.camera:    Camera
    ...
    def _update(self) -> None:
        ...
        for plat in self.platforms:
            if isinstance(plat, __import__("sprites.platform", fromlist=["MovingPlatform"]).MovingPlatform):
                plat.update()
    ...
    def _check_enemy_collisions(self) -> None:
        for enemy in list(self.enemies):
            if not enemy.stomped and self.player.rect.colliderect(enemy.rect):
                # Pisar de cima
                if self.player.vel_y > 0 and self.player.rect.bottom < enemy.rect.centery + 8:
                    enemy.get_stomped()
    ...
    def _draw(self) -> None:
        ...
        ox = self.camera.offset_x if self.state in (STATE_PLAYING, STATE_PAUSED) else 0
        ...
        if self.state in (STATE_PLAYING, STATE_PAUSED):
            ...
            self._draw_overlay(
                f"FASE {self.level_index} COMPLETA!",
    ...
    def _draw_world(self, ox: int) -> None:
        def blit_shifted(sprite: pygame.sprite.Sprite) -> None:
            r = sprite.rect.copy()
            r.x -= ox
